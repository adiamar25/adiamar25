...
public:
     ...
     */
    double get_average() const { return _average; }
...
std::ostream& operator<<(std::ostream& os, Student const& s) {
    return os << s.get_name() << " (average: " << s.get_average() << ")";
}
...
int main(int argc, const char* argv[]) {
    /* Setup */
    vl_vector<Student, STATIC_CAPACITY> students;
    std::string line;
    ...
    Student highest_grade_student;
    for (const auto& student : students) {
        if (student.get_average() > highest_grade_student.get_average()) {
    ...
    std::cout << "Student with highest grade: " << highest_grade_student << std::endl;
