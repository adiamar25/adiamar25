...
class Parser:
    ...
    def advance(self) -> None:
        ...
        # Your code goes here!
        self.__command_number += 1
        commentsless_command = self.remove_comments(
            self.__input_lines[self.__command_number])

        while commentsless_command == "":
            self.__command_number += 1
            commentsless_command = self.remove_comments(
                self.__input_lines[self.__command_number])
    ...
    def command_type(self) -> str:
        ...
        arithmetic_commands = ["add", "sub", "neg",
                               "eq", "gt", "lt", "and", "or", "not"]
        if self.__current_command[0].lower() in arithmetic_commands:
            return "C_ARITHMETIC"
        return "C_" + self.__current_command[0].upper()
    ...
    def arg2(self) -> int:
        ...
        """
        # Your code goes here!
        # relevant_commands = ["C_PUSH", "C_POP", "C_FUNCTION", "C_CALL"]
        # if self.__current_command in relevant_commands:
        return int(self.__current_command[2])

    def remove_spaces(self, command: str):
